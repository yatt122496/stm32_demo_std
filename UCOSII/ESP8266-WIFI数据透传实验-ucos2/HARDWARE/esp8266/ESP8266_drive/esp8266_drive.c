...
void ESP8266_Init(u32 bound)
{
	...
	RCC_APB2PeriphClockCmd(ESP8266_CH_PD_Pin_Periph_Clock, ENABLE);

//	GPIO_InitStructure.GPIO_Pin = ESP8266_RST_Pin;	   //PE6  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 	 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	 //IO口速度为50MHz
//	GPIO_Init(ESP8266_RST_Pin_Port, &GPIO_InitStructure);

  GPIO_InitStructure.GPIO_Pin = ESP8266_CH_PD_Pin;			   //PG7
...
int ESP8266_Rec_Data(char *str)
{
	...
	return ( strlen(ESP8266_Fram_Record_Struct .Data_RX_BUF ));
}
...
{
	ESP8266_Send_AT_Cmd ( "AT+RST", 0, 0, 500 );
	delay_ms(1000);
}
...
//pPassWord：WiFi密码字符串
//返回1：连接成功 0：连接失败
bool ESP8266_Creat_Wifi( char * pSSID, char * pPassWord )
{
	char cCmd [120];

	sprintf ( cCmd, "AT+CWSAP=\"%s\",\"%s\",1,3", pSSID, pPassWord );
...
//ComNum：服务器端口字符串
//返回1：连接成功 0：连接失败
bool ESP8266_Creat_Server(char * ComNum)
{
	char cStr [30];
	
	sprintf ( cStr, "AT+CIPSERVER=1,%s", ComNum);
...
//time：服务器等待超时时间
//返回1：连接成功 0：连接失败
bool ESP8266_Set_Time(int time)
{
	char cStr [30];
	
	sprintf ( cStr, "AT+CIPSTO=%d", time);
...
//id：连接设备的id号
//返回1：连接成功 0：连接失败
bool ESP8266_Send_Data(int id)
{
	char cStr [30];
	
	sprintf ( cStr, "AT+CIPSEND=%d,16", id);
