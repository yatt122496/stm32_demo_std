...
{  							  
	...
	if(!mode) //非叠加方式
	{
	    ...
	    {   
	        ...
	        for(t1=0;t1<8;t1++)
			{			    
		        if(temp&0x80)FRONT_COLOR=colortemp;
				else FRONT_COLOR=BACK_COLOR;
				LCD_DrawPoint(x,y);	
				temp<<=1;
				y++;
				if(x>=lcddev.width){FRONT_COLOR=colortemp;return;}//超区域了
				if((y-y0)==size)
				{
					y=y0;
					x++;
	...
	}else//叠加方式
	{
	    ...
	    {   
			...
			{			    
				...
				temp<<=1;
				y++;
				if(x>=lcddev.height){FRONT_COLOR=colortemp;return;}//超区域了
				if((y-y0)==size)
				{
					y=y0;
					x++;
...
void LCD_ShowFontHZ(u16 x, u16 y, u8 *cn)
{
	...
	while (*cn != '\0')
	{
		for (wordNum=0; wordNum<70; wordNum++)
		{	//wordNum扫描字库的字数
			if ((CnChar32x29[wordNum].Index[0]==*cn)
			     &&(CnChar32x29[wordNum].Index[1]==*(cn+1)))
			{
				...
				{	//MSK的位数
					...
					for(j=0;j<8;j++) 
					{
						if((color&0x80)==0x80)
						{
							LCD_Fast_DrawPoint(x,y,FRONT_COLOR);
						} 						
						else
						{
